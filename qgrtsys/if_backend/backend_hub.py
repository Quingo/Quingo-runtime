...
class Backend_info():
    ...
    def get_module(self):
        if self.with_timing:
            prefix = 'qgrtsys.if_backend.arch_backend.'
        else:
            prefix = 'qgrtsys.if_backend.non_arch_backend.'
...
class Backend_hub():
    def __init__(self):
        ...
        self.backends['pycactus_quantumsim'] = Backend_info(
            'Pycactus_quantumsim', 'pycactus.pycactus_quantumsim', with_timing=False,
            is_simulator=True)

        self.backends['pyqcisim_quantumsim'] = Backend_info(
            'PyQCISim_quantumsim', 'pyqcisim.pyqcisim_quantumsim', with_timing=False,
            is_simulator=True)
